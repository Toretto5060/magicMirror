...
let host ="mirror.toretto.top:3000/"
...
export const userMusic = params => {  //获取网易云指定用户的歌单
  return axiosIns.get('/user/playlist', {params:params}) 
}
export const userMusicDetails = params => { //获取歌单歌曲
  return axiosIns.get('/playlist/detail', { params: params}) 
}
export const userMusicUrl= params => { //获取歌曲url  若多个id，以,隔开
  return axiosIns.get('/song/url', {params: params})
}

export const cheackMusicUrl = params => { //查询歌曲是否可播放 返回success
  return axiosIns.get('/check/music', {params: params})
}
export const hotMusic = params => { //获取网易云指定用户的歌单
  return axiosIns.get('/top/list?idx=1', { params: params }) 
}
